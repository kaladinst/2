...
void DLinkedList::Crossover() {
    ...
    if (firstGenes.size() % 2 == 0 && secondGenes.size() % 2 == 0) {
        ...
        ofstream outFile("src/Dna.txt", ios::app);
        if (outFile.is_open()) {
            current = newFirst.head;
            while (current != nullptr) {
